...
class Dataset360D(Dataset):
    #360D Dataset#
    def __init__(self, filenamesFile, delimiter, mode, inputShape, vps_path,transform=None, rescaled=False):
        #########################################################################################################
        # Arguments:
    ...
    def loadItemUD(self, idx):
        ...
        if (idx >= self.length):
            ...
            upRGB = cv2.imread(self.sample["upRGB"][idx])
            upRGB = cv2.cvtColor(upRGB, cv2.COLOR_BGR2RGB)
            cube_leftRGB = self.e2c.run(leftRGB)

            dtmp = np.array(cv2.imread(self.sample["leftDepth"][idx], cv2.IMREAD_ANYDEPTH))        
            dtmp = cv2.resize(dtmp, (self.width, self.height))            
            ...
            cube_aug_rgb = self.normalize(self.pilToTensor(cube_leftRGB))
            aug_rgb = self.normalize(self.pilToTensor(leftRGB))
            ...
            inputs = {
                ...
                "normalized_rgb": aug_rgb,
                "normalized_cube_rgb": cube_aug_rgb,
