...
from losses import SVS, BerhuLoss, SVS_plane
...
class Trainer:
    def __init__(self, settings):
        ...
        train_dataset = self.dataset(settings.train_path, " ", "ud", [256, 512], settings.vps_path)
        ...
        val_dataset = self.dataset(settings.test_path, " ", "ud", [256, 512], settings.vps_path)
        ...
        #self.compute_loss = SVS(self.device, settings)
        self.compute_loss = SVS_plane(self.device, settings)
    ...
    def train_one_epoch(self, epoch):
        ...
        for batch_idx, inputs in enumerate(pbar):
            ...
            if early_phase or late_phase:
                ...
                pred_depth = outputs["pred_depth"].detach().to(self.device)
                gt_depth = inputs["leftDepth"].to(self.device)
                mask = inputs["val_mask"].to(self.device)
    ...
    def process_batch(self, inputs, epoch):
        ...
        losses["loss"] = self.compute_loss(inputs["rgb"].to(self.device),
                                           outputs["pred_depth"].to(self.device),
                                           inputs["upDepth"].to(self.device),
                                           inputs["upRGB"].to(self.device),
                                           inputs["vps"].to(self.device), epoch)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch_idx, inputs in enumerate(pbar):
                outputs, losses = self.process_batch(inputs, 100)
                pred_depth = outputs["pred_depth"].detach().to(self.device)
                gt_depth = inputs["leftDepth"].to(self.device)
                mask = inputs["val_mask"].to(self.device)
    ...
    def log(self, mode, inputs, outputs, losses):
        ...
        for j in range(min(4, self.settings.batch_size)):  # write a maxmimum of four images
            ...
            writer.add_image("gt_depth/{}".format(j),
                             inputs["leftDepth"][j].data/inputs["leftDepth"][j].data.max(), self.step)
